...
let decode_recode () =
  ...
  for i = 0 to 1000 do
    ...
    check float_eps "datetime" (get_int64 r 0 |> Int64.float_of_bits) r''.R.datetime;
...
let partial =
  (module Partial_testable : TESTABLE with type t = E.encode_result)
...
let encode_manual () =
  ...
  check partial "after adding empty buf, second await" `Partial (E.encode e Await);
...
let decode_smallbuf () =
  ...
  for i = 0 to len - 1 do
    ...
    if i < len - 1
    then check decode_result "intermediate decode" D.Await (D.decode d)
    else begin
...
let encode_smallbuf () =
  ...
  for i = 0 to len - 1 do
    ...
    if i < len - 1
    then check partial (Printf.sprintf "loop %d" i) `Partial (E.encode e Await)
...
let decode_recode_2b_smallbuf () =
  ...
  for i = 0 to H.size + R.size - 1 do
    ...
    check partial "encode r1" `Partial (E.encode e Await);
  ...
  for i = H.size + R.size to H.size + 2 * R.size - 1 do
    ...
    if i = H.size + 2 * R.size - 1 then
      ...
      check partial "encode r2" `Partial (E.encode e Await);
    Bytes.(set h_plus_2b' i @@ get b 0);
  done;
...
let decode_encode_3pages () =
  ...
  check int "buf size" (3 * io_buffer_size - ((3 * io_buffer_size - H.size) mod R.size)) (String.length dst);
  check string ""
...
let basic = [
  ...
  test_case "chk_hdr" `Quick chk_hdr;
