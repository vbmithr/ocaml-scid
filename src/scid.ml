  ...
  let check b pos =
    try
      for i = pos to pos + size - 1 do
        if not @@ char_valid b.[i] (i - pos) then
          failwith (Printf.sprintf "Char at pos %d should not be %C" i b.[i])
      done ;
      Ok ()
...
module R = struct
  ...
  let pp fmt
    ...
    Format.fprintf fmt ".2%f %f %f %f %f %Ld %Ld %Ld %Ld" datetime o h l c
  ...
  let read_bytes buf pos = read (Bytes.unsafe_to_string buf) pos

  let write r buf pos =
    let open EndianBytes.LittleEndian in
    let set_int64_uint32 tmpbuf b p i =
      set_int64 tmpbuf 0 i ; Bytes.blit tmpbuf 0 b p 4 in
  ...
  let compare r r' =
    ...
    let b' = Bytes.make size '\000' in
    write r b 0 ; write r' b' 0 ; compare b b'
...
module D = struct
  ...
  type error = Header_invalid of string | Eof of string
  ...
  module Manual = struct
    let refill_string d b p l =
      ...
      if l = 0 then eoi d else d.buf <- Bytes.unsafe_of_string b ;
      d.pos <- p ;
      d.max <- p + l - 1

    let refill_bytes d b p l =
      if p < 0 || l < 0 || p + l > Bytes.length b then invalid_arg "bounds" ;
      if l = 0 then eoi d else d.buf <- b ;
      d.pos <- p ;
      d.max <- p + l - 1
  end

  let refill : type a. (a t -> decode_result) -> a t -> decode_result =
   fun k d ->
    match d.src with
    | String _ -> eoi d ; k d
    | Bytes _ -> eoi d ; k d
    ...
    | Channel ic ->
        ...
        Manual.refill_bytes d d.buf 0 rc ;
        k d
  ...
  let make : type a. a src -> a t =
   fun src ->
    let buf, pos, max =
      ...
      | Bytes b -> (b, 0, Bytes.length b - 1) in
    make' src buf pos max

  let of_string s =
    make' (String s) (Bytes.unsafe_of_string s) 0 (String.length s - 1)

  let of_bytes b = make' (Bytes b) b 0 (Bytes.length b - 1)
  let of_channel ch = make' (Channel ch) (Bytes.create io_buffer_size) 1 0
  let manual () = make' Manual (Bytes.create 0) 1 0
  let decode d = d.k d
end
...
module E = struct
  ...
  module Manual = struct
    let add_bytes e buf pos max =
      if pos < 0 || max < 0 || pos + max > Bytes.length buf then
        invalid_arg "bounds"
      else e.buf <- buf ;
      e.pos <- pos ;
      e.max <- pos + max - 1
  ...
  let flush : type a. (a t -> encode_result) -> a t -> encode_result =
   fun k e ->
    match e.dst with
    | Manual ->
        e.k <- partial k ;
        `Partial
    | Buffer b ->
        Buffer.add_subbytes b e.buf 0 e.pos ;
        e.pos <- 0 ;
        k e
    | Channel oc ->
        output oc e.buf 0 e.pos ;
        e.pos <- 0 ;
        k e
  ...
  let rec encode_h k e =
    ...
    if can_write < 1 then flush (encode_h k) e
      ...
      assert (e.st <> `R) ;
      if e.p_pos = 0 && can_write >= H.size then (
        H.write e.buf e.pos ;
        e.pos <- e.pos + H.size ;
        e.st <- `R ;
        k e )
  ...
  let rec encode_r k r e =
    ...
    if can_write < 1 then flush (encode_r k r) e
      ...
      assert (e.st = `R) ;
      if e.p_pos = 0 && can_write >= R.size then (
        R.write r e.buf e.pos ;
        e.pos <- e.pos + R.size ;
        k e )
      else
  ...
  let _encode k e v =
    ...
    | End -> flush k e
    | Await -> k e
    | R r -> if e.st = `H then encode_h (encode_r k r) e else encode_r k r e
  ...
  let make : type a. a dst -> a t =
   fun dst ->
    let buf, pos, max =
      match dst with
      | Manual -> (Bytes.create 0, 1, 0)
      | Channel _ -> (Bytes.create io_buffer_size, 0, io_buffer_size - 1)
      | Buffer _ -> (Bytes.create io_buffer_size, 0, io_buffer_size - 1) in
